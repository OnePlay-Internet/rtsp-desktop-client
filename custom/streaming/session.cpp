...
void
handle_server_infor(ServerInfor* infor, void* data)
{
    ...
    int riKeyId;
    memcpy(&riKeyId, session->m_StreamConfig.remoteInputAesIv, sizeof(riKeyId));
    riKeyId = qFromBigEndian(riKeyId);
    auto rki = std::to_string(riKeyId);

    LaunchRequest request = {0};
    memcpy(request.rikey,session->m_StreamConfig.remoteInputAesKey, sizeof(session->m_StreamConfig.remoteInputAesKey));
    memcpy(request.rikeyid,rki.data(), rki.size());
    memcpy(request.appid,"app",3);
    request.localAudioPlayMode = true;
    SendLaunchRequest(session->m_Signaling,&request);
}

void
handle_launch_response(LaunchResponse* res, void* data)
{
    Session* session = (Session*)data;
    session->m_RTSPurl = QString(res->sessionUrl);
    session->m_RTSPReady = true;
}

Session::Session(QString token, 
...
{
    GrpcConfig config { "\0", "localhost", 6000, };
    memcpy(config.token,token.toStdString().data(),token.toStdString().size());
    m_Signaling = new_signaling_client(config,
        handle_server_infor,
        NULL,
